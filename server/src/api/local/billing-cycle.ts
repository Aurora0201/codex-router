import type { BillingCadence } from "../../types.js";
...
/**
 * The current or next billing date; the billing day itself still counts as
 * current. Mirrors web/src/lib/billing-cycle.ts, which renders the same date
 * in the console — the two are locked together by billing-cycle.test.ts and
 * should move to a shared module if a third caller appears.
 */
export function nextBillingAt(
  ...
  if (cadence === "annual") {
    ...
    if (candidate < today.getTime()) candidate = utcDate(year + 1, anchor.getUTCMonth(), anchorDay);
    return candidate;
  }

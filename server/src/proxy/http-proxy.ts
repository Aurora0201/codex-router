...
import { safeResponseMetadata } from "./response-metadata.js";
...
interface HttpProxyOptions {
  ...
  /** A 429 is proof the account cannot serve, so the pool is re-examined. */
  onRateLimited?(accountId: string): void;
}

function errorStatus(error: unknown): number {
  switch ((error as Error).message) {
    case "no_active_account_selected":
      return 503;
    case "account_disabled":
    case "account_not_ready":
    case "fedramp_accounts_not_supported":
      return 409;
    default:
      return 502;
  }
...
export class HttpProxy {
  ...
  async handle(request: FastifyRequest, reply: FastifyReply, path: ProxyPath): Promise<void> {
    ...
    const transport: Transport =
      path === "/models"
        ? "models"
        : path === "/responses/compact" || (path === "/responses" && isCompactionRequest(request.headers))
          ? "compact"
          : path === "/alpha/search"
            ? "search"
            : "http";
    const logId = this.options.database.requestLog.startRequest({
    ...
    if (hasBrowserOrigin(request)) {
      this.options.database.requestLog.finishRequest(
        logId,
        gatewayFailure("browser_origin_not_allowed", 403, "routing"),
      );
    ...
    let identityMode: IdentityMode = "managed_account";
    let clientCancelled = false;
    let failureStage: "sending" | "streaming" = "sending";
    let metadata: Pick<RequestEvidence, "httpStatus" | "upstreamRequestId" | "diagnosticHeaders"> = {};
    let bytesOut = 0;
    let terminal: RequestEvidence | null = null;
    const finish = (evidence: RequestEvidence) =>
      this.options.database.requestLog.finishRequest(logId, {
        ...evidence,
        ...metadata,
    ...
    try {
      ...
      selectedAccount = identity.mode === "managed_account" ? identity.account : null;
      if (selectedAccount?.fedRamp) throw new Error("fedramp_accounts_not_supported");
      this.options.database.requestLog.setContext(logId, {
      ...
      const send = () =>
        undiciRequest(this.upstreamUrl(request, path), {
          method: request.method as Dispatcher.HttpMethod,
          headers: credential
            ? buildUpstreamHeaders(request.headers, credential, request.method === "GET" ? undefined : rawBody.length)
            : buildClientPassthroughHeaders(request.headers, request.method === "GET" ? undefined : rawBody.length),
      ...
      metadata = {
        ...
        ...safeResponseMetadata(upstream.headers as Record<string, string | string[] | undefined>),
      };
      if (upstream.statusCode === 401 && selectedAccount && credential) {
        await upstream.body.dump();
        credential = await this.options.auth.refresh(selectedAccount.id);
        upstream = await send();
      }
      if (upstream.statusCode === 429 && selectedAccount) {
        void this.options.usage.refreshInBackground(selectedAccount.id);
        this.options.onRateLimited?.(selectedAccount.id);
      }
      const headers = upstream.headers as Record<string, string | string[] | undefined>;
      metadata = { httpStatus: upstream.statusCode, ...safeResponseMetadata(headers) };
      failureStage = "streaming";
      copyResponseHeaders(headers, reply.raw);
      reply.hijack();
      ...
      const inspector = inspectResponses
        ? new ResponsesSseInspector((event) => {
            terminal = classifyProtocolTerminal(
              event.type ?? "",
              event.errorCode ?? event.incompleteReason,
              event.status,
            );
      ...
      const gateway = [
        "no_active_account_selected",
        "account_disabled",
        "account_not_ready",
        "fedramp_accounts_not_supported",
      ...
      finish(
        terminal ??
          (clientCancelled
            ? clientCancellation()
            : gateway
              ? gatewayFailure(rawCode, status, rawCode.includes("account") ? "routing" : "sending")
              : {
                  ...transportFailure(transportError.diagnosticCode, failureStage),
                  transportErrorChain: transportError.transportErrorChain,
                }),
      );
      if (clientCancelled) return;
      if (!reply.raw.headersSent)
        await reply.code(status).send({ error: gateway ? rawCode : "upstream_request_failed" });
      else reply.raw.destroy(error as Error);
    }
  }
  private upstreamUrl(request: FastifyRequest, path: ProxyPath) {
    const base = `${this.options.upstreamBaseUrl}${path}`;
    const raw = request.raw.url ?? "";
    const index = raw.indexOf("?");
    return index === -1 ? base : `${base}${raw.slice(index)}`;
  }
  private rawBody(body: unknown) {

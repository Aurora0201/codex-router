...
const [rootPackage, serverPackage, webPackage, lockfile] = await Promise.all([
  ...
  readJson("web-v2/package.json"),
  readJson("package-lock.json"),
]);

const versions = new Set([rootPackage.version, serverPackage.version, webPackage.version]);
if (versions.size !== 1) throw new Error("root, server and web-v2 versions must match");
...
if (lockfile.packages?.["web-v2"]?.version !== webPackage.version) throw new Error("web-v2 lockfile version is out of sync");

...
export function formatUsageWindow(window: UsageWindowView | null): string {
  ...
  if (minutes % 10080 === 0) return i18n.t("{{count}} 天额度", { count: (minutes / 10080) * 7 })
  if (minutes % 1440 === 0) return i18n.t("{{count}} 天额度", { count: minutes / 1440 })
  if (minutes % 60 === 0) return i18n.t("{{count}} 小时额度", { count: minutes / 60 })
...
export function formatRelativeTime(
  ...
  if (diff === 0) {
    ...
    if (elapsed < 3_600_000) return i18n.t("{{count}} 分钟前", { count: Math.floor(elapsed / 60_000) })
    return i18n.t("{{count}} 小时前", { count: Math.floor(elapsed / 3_600_000) })
  }
  if (diff < 3_600_000) return i18n.t("{{count}} 分钟后", { count: Math.ceil(diff / 60_000) })
  if (diff < 86_400_000) return i18n.t("{{count}} 小时后", { count: Math.ceil(diff / 3_600_000) })

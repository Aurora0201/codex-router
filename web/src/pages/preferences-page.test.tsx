...
describe("PreferencesPage", () => {
  it("renders local environment actions and copies their directory paths", async () => {
    ...
    expect(screen.getByRole("link", { name: "Codex Router 版本" })).toHaveAttribute(
      "href",
      "https://github.com/Aurora0201/codex-router"
    )
    expect(screen.getByRole("link", { name: "Codex Router 版本" })).toHaveAttribute("target", "_blank")
    expect(screen.getByRole("link", { name: "Codex Router 版本" })).toHaveAttribute("rel", "noopener noreferrer")
    await userEvent.click(screen.getByRole("button", { name: "数据目录" }))
    await waitFor(() => expect(writeText).toHaveBeenCalledWith(service.snapshot.health.dataDir))
    expect(screen.getByText("目录路径已复制")).toBeInTheDocument()
    await userEvent.click(screen.getByRole("button", { name: "Codex 配置备份" }))
    await waitFor(() => expect(writeText).toHaveBeenCalledWith("C:\\Users\\test\\.codex"))
    await userEvent.click(screen.getByRole("button", { name: "Codex Router 运行日志" }))
    await waitFor(() => expect(writeText).toHaveBeenCalledWith("C:\\Users\\test\\.codex-router\\logs"))
    ...
    expect(dataItem?.querySelector('[data-slot="item-actions"]')).toHaveClass("self-center")
    expect(screen.getByRole("switch", { name: "请求元数据记录" }).closest('[data-slot="field"]')).toHaveClass("items-center!")

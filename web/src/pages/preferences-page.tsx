...
import { CopyIcon, DatabaseIcon, ExternalLinkIcon, FileArchiveIcon, FileTextIcon, LanguagesIcon, MonitorIcon, MoonIcon, PackageIcon, SunIcon } from "lucide-react"
...
function EnvironmentItem({ title, description, icon: Icon, href, disabled = false, onActivate }: EnvironmentItemProps) {
  const render = href
    ? <a href={href} target="_blank" rel="noopener noreferrer" aria-label={title} />
    : <button type="button" disabled={disabled} onClick={onActivate} aria-label={title} />
  return (
    <Item variant="outline" render={render} className="min-w-0 flex-nowrap text-left enabled:cursor-pointer enabled:hover:border-ring/40 enabled:hover:bg-muted/50 disabled:cursor-not-allowed disabled:opacity-60">
      ...
      <ItemActions className="self-center text-muted-foreground [&_svg]:size-4">{href ? <ExternalLinkIcon aria-hidden="true" /> : <CopyIcon aria-hidden="true" />}</ItemActions>
    </Item>
  )
}
...
export function PreferencesPage({ snapshot, service, reload, onThemeChange }: { snapshot: GatewaySnapshot; service: GatewayService; reload(): Promise<void>; onThemeChange(theme: Theme): Promise<void> }) {
  ...
  const logDirectory = snapshot.health.logFilePath ? parentDirectory(snapshot.health.logFilePath) : null
  const copyPath = async (path: string) => {
    try {
      await navigator.clipboard.writeText(path)
      toast.add({ title: t("目录路径已复制"), description: path, type: "success" })
    } catch (error) {
      toast.add({ title: t("无法复制目录路径"), description: (error as Error).message, type: "error" })
    }
  }

...
function ConnectionStatus({
  ...
  connection: WebSocketConnectionView
}) {
  const { t } = useTranslation()
  if (connection.state === "connecting")
    return (
      <span role="status" className="shimmer text-warning">
        {t("连接中")}
      </span>
    )
  if (connection.state === "retiring")
    return (
      <span role="status" className="block shimmer truncate text-warning">
        {t("正在退役")} · {t("等待当前请求结束")}
      </span>
    )
  if (connection.state === "transmitting")
    return (
      <span role="status" className="shimmer text-success">
        {t("正在传输")}
      </span>
    )
...
function activityLabel(
  ...
  const kind =
    connection.activityKind === "compaction"
      ? t("上下文压缩")
...
function ConnectionIdentifiers({
  connection,
}: {
  connection: WebSocketConnectionView
}) {
  ...
  return (
    <div className="flex max-w-80 flex-col gap-1">
      <span>
        {t("对话")}: {connection.threadId ?? "—"}
      </span>
      <span>Turn: {connection.turnId ?? "—"}</span>
      <span>Session: {connection.sessionId ?? "—"}</span>
      <span>
        {t("连接")}: {connection.connectionId}
      </span>
      <span>
        {t("请求标识")}: {connection.activeRequestId ?? "—"}
      </span>
    </div>
  )
}
...
function AnimatedCellValue({
  ...
  return (
    <div ref={ref} data-slot="animated-cell-value" className="min-w-0 truncate">
      {children}
    </div>
  )
}
...
export function WebSocketActivityCard({
  ...
  const [order, setOrder] = useState<ConnectionOrder>(() =>
  ...
  const orderedConnections = useMemo(() => {
    ...
    return [
      ...ordered,
      ...connections.filter(
        (connection) => !known.has(connection.connectionId)
      ),
    ]
  ...
  useLayoutEffect(() => {
    ...
    for (const [connectionId, row] of rowRefs.current) {
      ...
      if (!previousRect) {
        const animation = row.animate([{ opacity: 0 }, { opacity: 1 }], {
          duration: 160,
          easing: "ease-out",
        })
        rowAnimations.current.set(connectionId, animation)
        const cleanup = () => {
          if (rowAnimations.current.get(connectionId) === animation)
            rowAnimations.current.delete(connectionId)
        }
  ...
  const transmitting = connections.filter(
    (connection) => connection.state === "transmitting"
  ).length
  return (
    <Panel
      title={t("实时连接")}
      icon={RadioIcon}
      hint={t("{{connections}} 条连接 · {{transmitting}} 条传输中", {
    ...
    >
      <div
        ...
        aria-label={t("WebSocket 实时传输") as string}
      >
        {connections.length === 0 ? (
          <Empty className="h-full border-0 bg-transparent">
            <EmptyHeader>
              ...
              <EmptyTitle>{t("当前没有 WebSocket 连接")}</EmptyTitle>
              <EmptyDescription>
                {t("Codex 建立连接后会显示在这里。")}
              </EmptyDescription>
            </EmptyHeader>
          ...
          <>
            <ScrollArea
              className="h-full overscroll-contain [&_[data-slot=scroll-area-viewport]]:[scroll-padding-bottom:--spacing(2)] [&_[data-slot=scroll-area-viewport]]:[contain:paint] [&_[data-slot=scroll-area-viewport]]:[overflow-anchor:none] [&_[data-slot=table-container]]:overflow-visible"
            ...
            >
              <Table className="min-w-[44rem] table-fixed text-sm">
                <TableHeader>
                  <TableRow>
                    <TableHead className="sticky top-0 z-10 w-[28%] bg-muted pl-4">
                      {t("对话")}
                    </TableHead>
                    <TableHead className="sticky top-0 z-10 w-[37%] bg-muted">
                      {t("当前活动")}
                    </TableHead>
                    <TableHead className="sticky top-0 z-10 w-[20%] bg-muted">
                      {t("状态")}
                    </TableHead>
                    <TableHead className="sticky top-0 z-10 w-[15%] bg-muted">
                      {t("连接时间")}
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {orderedConnections.map((connection) => (
                    <TableRow
                      ...
                      ref={(row) => {
                        if (row)
                          rowRefs.current.set(connection.connectionId, row)
                    ...
                    >
                      <TableCell className="pl-4 align-middle">
                        <Tooltip>
                          ...
                          >
                            <ConversationLabel connection={connection} />
                          </TooltipTrigger>
                          <TooltipContent>
                            <ConnectionIdentifiers connection={connection} />
                          </TooltipContent>
                        </Tooltip>
                      </TableCell>
                      <TableCell>
                        <AnimatedCellValue
                          value={`${connection.turnId ?? ""}:${connection.activityKind ?? ""}`}
                        >
                          <Tooltip>
                            <TooltipTrigger
                              render={<span className="block truncate" />}
                            >
                              {activityLabel(connection, t)}
                            </TooltipTrigger>
                            <TooltipContent>
                              <ConnectionIdentifiers connection={connection} />
                            </TooltipContent>
                      ...
                      <TableCell>
                        <AnimatedCellValue value={connection.state}>
                          <ConnectionStatus connection={connection} />
                        </AnimatedCellValue>
                      </TableCell>
                      <TableCell>
                        <span className="text-muted-foreground tabular-nums">
                          {connectedDuration(connection.connectedAt, now)}
                        </span>
                      </TableCell>
                    </TableRow>
                  ))}
